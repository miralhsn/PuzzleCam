...
// Module-level singleton — one MediaPipe instance per page load
let globalLandmarker: any = null;
let globalInitPromise: Promise<any> | null = null;

async function getOrInit(onMsg: (m: string) => void): Promise<any> {
  ...
  globalInitPromise = (async () => {
    ...
    // Dynamic import — works correctly with webpack (--turbo false)
    ...
    for (const url of MODEL_URLS) {
      try {
        ...
        if (r.ok) { modelBuffer = await r.arrayBuffer(); break; }
      } catch { /* try next */ }
    }
    if (!modelBuffer || modelBuffer.byteLength < 1000) {
      throw new Error(
        'Could not download hand model. Check your internet connection.'
      );
    }
...
export function useHandTracking(onResults: HandResultsCb) {
  ...
  useEffect(() => {
    let mounted = true;
    getOrInit(msg => { if (mounted) setLoadingMsg(msg); })
      .then(() => {
  ...
  function detect(video: HTMLVideoElement, timestampMs: number): void {
    ...
    const lms: HandLandmark[][] = (results.landmarks ?? []).map(
      (hand: any[]) =>
        hand.map((pt: any) => ({ x: pt.x, y: pt.y, z: pt.z ?? 0 }))
    );
    cbRef.current(lms);
  }
